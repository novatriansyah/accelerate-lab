...
<a href="/contact" class="bg-primary hover:bg-primary-dark text-white text-lg font-semibold px-8 py-4 rounded-lg shadow-lg shadow-primary/30 transition-all hover:scale-105 flex items-center justify-center gap-2 group">
                            Start Your Project
                            <span class="material-icons-round group-hover:translate-x-1 transition-transform">arrow_forward</span>
</a>
<a href="/case-studies" class="bg-white dark:bg-slate-800 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 hover:border-primary text-lg font-semibold px-8 py-4 rounded-lg transition-all hover:shadow-md flex items-center justify-center gap-2">
                            View Case Studies
                        </a>
</div>
<div class="mt-12 flex items-center justify-center lg:justify-start gap-8 border-t border-slate-200 dark:border-slate-800 pt-8 opacity-80">
    @if(isset($heroStats) && count($heroStats) > 0)
        ...
        <div>
            <p class="text-3xl font-bold text-slate-900 dark:text-white">{{ $stat->value }}<span class="text-primary text-xl">{{ $stat->unit }}</span></p>
...
<a class="inline-flex items-center text-primary font-semibold mt-8 group-hover:translate-x-2 transition-transform" href="/services">
...
<div class="grid grid-cols-1 sm:grid-cols-3 w-full gap-8 divide-y sm:divide-y-0 sm:divide-x divide-slate-100 dark:divide-slate-700">
    @if(isset($capabilityStats) && count($capabilityStats) > 0)
        ...
        <div class="text-center pt-4 sm:pt-0">
            <p class="text-4xl font-extrabold text-slate-900 dark:text-white mb-1">{{ $stat->value }}<span class="text-primary text-2xl ml-1">{{ $stat->unit }}</span></p>
            <p class="text-sm text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wide">{{ $stat->label }}</p>
        </div>
        @endforeach
    @else
        ...
        <div class="p-8 flex flex-col flex-grow">
            ...
            <div class="space-y-4 mb-8 flex-grow">
                <div>
                    ...
                    <p class="text-sm text-slate-600 dark:text-slate-400 line-clamp-2">{{ strip_tags($project->challenge) }}</p>
                </div>
                <div>
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Solution</p>
                    <p class="text-sm text-slate-600 dark:text-slate-400 line-clamp-2">{{ strip_tags($project->solution) }}</p>
                </div>
            </div>
